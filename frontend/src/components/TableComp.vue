...
const { columns, data, deleteData, editData } = defineProps({
  columns: {
    type: Array,
    default: () => [],
  },
    ...
    <table class="table table-striped-columns table-bordered">
      ...
      <tbody>
        <tr v-for="(d, i) in data" :key="i">
          ...
          <td v-for="(c, ci) in columns" :key="ci" :class="[
          ...
          ]" :style="d[ci] === 'delete' || d[ci] === 'edit' ? { cursor: 'pointer' } : {}"
            ...
            <div v-if="c.toLowerCase().includes('image')">
              <img :src="d[ci]" alt="profile-image" class="img-thumbnail rounded float-start" width="100" height="100">
            </div>
