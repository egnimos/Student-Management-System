...
async function getStudentEnrollements() {
    try {
        enrollmentsData.isLoading = true;
...
<template>
    <br>
    <DetailsCard :title="studentData.name" :image="studentData.image" 
    :subtitle="studentData.email" :description="`${studentData.mobile_number}`"/>
    <br>
    ...
    <hr>
    <TableComp :columns="columns" :data="enrollmentsData.data" 
    :deleteData="deleteEnrollement" :editData="editEnrollement" />
</template>
